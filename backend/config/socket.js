  ...
  io.on("connection", (socket) => {
    ...
    socket.on("buttonClick", (button) => {
      ...
      if (waitingAgreeUsers.length > 0 && waitingDisagreeUsers.length > 0) {
        ...
        if (disagreeSocket) {
          console.log(
            `[${new Date().toLocaleString()}] Users ${
              agreeSocket.username
            } and ${disagreeSocket.username} have been matched.`
          );
         ...
         await processAudioData(data.audioBuffer);
          ...
          agreeSocket.on("disconnect", () => {
            console.log(
              `User ${agreeSocket.id} has disconnected from the chat.`
            );
            io.to(roomName).emit("chatMessage", {
              username: "System",
    ...
    socket.on("disconnect", () => {
      ...
      const index1 = waitingAgreeUsers.findIndex(
